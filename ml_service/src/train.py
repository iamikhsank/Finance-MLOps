...
def train_model():
    ...
    print("Loading data for Optuna HPO...")
    ...
    def objective(trial):
        ...
        model = StockLSTM(input_size=6, hidden_size=hidden_size, num_layers=num_layers, output_size=1)
    ...
    # Now train final model with best params
    ...
    model = StockLSTM(input_size=6, hidden_size=best_hidden_size, num_layers=best_num_layers, output_size=1)
    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=best_lr)
    
    print("Starting MLflow run for best model...")
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        price_accuracy = max(0, 100 * (1 - mape))
        ...
        message_body = (
            ...
            f"RMSE:                  {rmse:.4f}\n"
            f"MSE:                   {mse:.4f}\n\n"
            f"-- Training Hyperparameters (Optuna Best) --\n"
            f"Sequence Length:       {seq_length}\n"
            f"Batch Size:            {batch_size}\n"
            f"Epochs:                {num_epochs}\n"
            f"Learning Rate:         {best_lr:.6f}\n"
            f"Hidden Size:           {best_hidden_size}\n"
            f"Num Layers:            {best_num_layers}\n\n"
